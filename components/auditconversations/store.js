const {auditconversationSerFinanzaModel,
    ...
    auditconversationIgsDaviviendaCRModel} = require('./model');
...
async function getConversations(filterConversations){
    ...
    switch (nameDB) {
        ...
        case 'igsBancolombiaAM' :
                ...
                return auditconversation;
        case 'igsAlmacenesSI' :
                auditconversation = await auditconversationIgsAlmacenesSIModel.find(filter);
                return auditconversation;
        case 'igsJelpitCO' :
                auditconversation = await auditconversationIgsJelpitCOModel.find(filter);
                return auditconversation;
        case 'igsComfamiliarCO' :
                auditconversation = await auditconversationIgsComfamiliarCOModel.find(filter);
                return auditconversation;
        case 'igsBancoAgricolaSV' :
                auditconversation = await auditconversationIgsBancoAgricolaSVModel.find(filter);
                return auditconversation;
        case 'igsDaviviendaCR' :
                auditconversation = await auditconversationIgsDaviviendaCRModel.find(filter);
                return auditconversation;                                    
        }
